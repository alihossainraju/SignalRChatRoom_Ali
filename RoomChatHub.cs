...
using SignalRChatRoom_Ali.Models;
    ...
    public class ChatRoomHub : Hub
    {
        ...
        public string Login(string loginName, string loginPassword)
        {
            ...
            else
            {
                ...
                {
                    var userRoomes = context.MeetingRooms.Include(CreateDatabaseIfNotExists => CreateDatabaseIfNotExists.UserRole).Where(cr => cr.UserRole.UserName == loginName && cr.Allowed == true).Select(cr => cr.RoomName).ToArray();
        ...
        public void Logout(string loginName)
        {
            ...
            List<string> joinedRooms = context.MeetingRooms.Include(CreateDatabaseIfNotExists => CreateDatabaseIfNotExists.UserRole).Where(cr => cr.UserRole.UserName == loginName && cr.Allowed == true).Select(cr => cr.RoomName).ToList();
        ...
        {
            var chatRooms = context.RoomChats.Select(cr => new { cr.RoomName }).ToArray();
            return chatRooms;
        }

        public object GetAllowedRoomsForUser(string userName)
        {
            var allowedChatRoomsForUser = context.MeetingRooms.Include(cr => cr.UserRole).Where(cr => cr.UserRole.UserName == userName).Select(cr => cr.RoomName).ToArray();
        ...
        {
            int countConversationRoom = context.MeetingRooms.Where(cr => cr.RoomName == roomName && cr.UserRoleID == userRoleId).Count();
            ...
            {
                MeetingRoom conversationRoom = new MeetingRoom(roomName, userRoleId, allowed);
                context.MeetingRooms.Add(conversationRoom);
                context.SaveChanges();
                var roomWiseUsers = context.MeetingRooms.Include(cr => cr.UserRole).Select(cr => new { cr.RoomName, cr.UserRole.UserName }).OrderBy(cr => cr.RoomName).ThenBy(cr => cr.UserName).ToArray();
